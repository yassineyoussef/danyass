...
/* Game Elements */
var tank, enemy, tanks= [], murs, mur1, briques = [], tirs = [], tousLesMurs=[], niveaux = [], etatJeu = {niveau:0,nom:"",gagnant:false },ordreConnexion=0;
var inputStates = {};
...
Tank.prototype.move = function() {		
	if(!this.enemy) {	
		...
		//console.log("villian moved " + inputStates['a']);
		...
		else if(inputStates['a']) { /*console.log("enemie va bouger gauche"); */this.moveWest(); }
...
function mainLoop(time) {
	if(etatJeu.nom == "scores") {
		...
		ctx.fillText(text + tanks[0].nom,100,200);
		ctx.fillStyle = "darkorange";
		ctx.restore();
		if(inputStates.enter) {
			etatJeu.scores = false;
			etatJeu.gagnant = false;
			passerNiveauProchain();
			clearCanvas();
		}
	...
	if(tanks.length == 1 && etatJeu.nom == "jouable") {
		var message = "the winner is " + tanks[0].nom
		console.log(message);
		var utterance = new SpeechSynthesisUtterance(message);
		window.speechSynthesis.speak(utterance);
		etatJeu.gagnant = tanks[0].nom;
		etatJeu.nom = "scores";
	}
	//console.log(socket);
	requestAnimationFrame(mainLoop);
...
function creerMursDuNiveau(n) {
	briques = [];
	for(var i = 0; i < tousLesMurs[n].murs.length; i++) {
		var m = tousLesMurs[n].murs[i];
		new Mur(m.x,m.y,m.lig,m.col);
	}
}

function passerNiveauProchain() {
	if(++etatJeu.niveau > 3) {
		etatJeu.niveau = 0;
	}
	console.log("va creer murs niveau " + etatJeu.niveau);
	creerMursDuNiveau(etatJeu.niveau);
	tank = new Tank(cw/2, ch-42-3, "Ryu","#3523af");
	enemy = new Tank(cw/2, 0,"Ken","#dd5554");
	enemy.direction = 'S';
	if(ordreConnexion == 1) {
		tank.enemy = false;
		enemy.enemy = true;
	} else if(ordreConnexion == 2) {
		tank.enemy = true;
		enemy.enemy = false;
	}
	tanks = [tank, enemy];
...
function start() {
	...
	tousLesMurs = [{murs: [{x:100,y:100,lig:5, col:3}, {x:300, y:200,lig:3,col:1},{x:400, y:50, lig:4, col:5},{x:0,y:200, lig:3, col:1}, {x:0,y:400, lig:3,col:3},{x:300,y:400,lig:3,col:3},{x:280 , y:280 , lig:2 ,col:2}]},
				   {murs: [{x: 40, y: 60, lig:1, col:6}, {x: 400, y:300, lig:2, col: 6}, {x: 100, y: 230, lig: 3, col:5}, {x: 360, y: 360, lig:4,col: 2}]},
				   {murs: [{x: 50, y: 100, lig:3, col:2}, {x: 250, y:300, lig:4, col: 6}, {x: 420, y: 130, lig:5, col:2}, {x: 160, y: 460, lig:4,col: 1}, {x: 80, y: 200, lig: 3, col: 8}]},
				   {murs: [{x: 300, y: 40, lig:7, col:6}, {x: 100, y:350, lig:2, col: 7}, {x: 0, y: 0, lig: 6, col:5}, {x: 360, y: 360, lig:4,col: 2}]},

				   ];
	
	// Initialise briques de niveau 0
	creerMursDuNiveau(0);
	...
	window.addEventListener('keydown', function(event) {
		switch(event.keyCode) {
			case 37 : inputStates.left = true; socket.emit('gauche') ;break;
			case 38 : inputStates.up = true; socket.emit('haut'); break;
			case 39 : inputStates.right = true; socket.emit('haut');break;
			case 40 : inputStates.down = true; socket.emit('bas'); break;
			case 32 : inputStates.space = true; socket.emit('tir');break;
...
function findTank(nom) {
	return tanks.find(function(t) {
		return t.nom == nom;
	});
}


function appuieEnemyTouche(touche) {
	inputStates[touche] = true;
	//console.log(inputStates);
	/*setTimeout(function() {
		inputStates[touche] = false;
	}, 1200);*/
...
socket.on('connect',function() {
	socket.emit('adduser');
	//console.log("emitted adduser");
});



socket.on('firsttank', function(data) {
	tank.enemy = false;
	enemy.enemy = true;
	tanks.push(tank);
	console.log("firsttank added");
	ordreConnexion= 1;
});

socket.on('secondtank', function(data) {
	tank.enemy = true;
	enemy.enemy = false;
	etatJeu.nom = "jouable";
	tanks.push(tank);
...
socket.on('enemygauche', function(data){
	appuieEnemyTouche('a');
	//console.log('enemygauche recu');
});

socket.on('enemydroite', function(data){
	appuieEnemyTouche('d');
}); 

socket.on('enemyhaut', function(data){
	appuieEnemyTouche('w');
});

socket.on('enemybas',function(data) {
	appuieEnemyTouche('s');
});

socket.on('enemytir',function(data) {
	appuieEnemyTouche('j');
});
